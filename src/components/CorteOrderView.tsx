...
type CorteStatus = typeof CORTE_STATUSES[number];

export function CorteOrderView({ order: initialOrder, onBack }: { order: Order; onBack: () => void }) {
  ...
  const [bulkStatus, setBulkStatus] = useState<'' | CorteStatus>('');
  ...
  const handleScanSubmit = useCallback(async () => {
    ...
    try {
      ...
      else if (!CORTE_STATUSES.includes(found.status as any)) toast.error(`Pedido em "${found.status}" — fora do corte`);
  ...
  // RPC
  const callRpc = async (id: string, novoStatus: CorteStatus, justificativa?: string) => {
  ...
  const baixar = async (novoStatus: CorteStatus, justificativa?: string) => {
    setActing(true);
  ...
  const handleEntradaClick = () => {
    if (order.status === 'Baixa Corte') setJustifyOpen(true);
    else baixar('Corte');
  };
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="min-w-0">
            ...
            {count > 0 && (
              <div className="mb-4 p-3 bg-muted rounded-lg flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <Select value={bulkStatus} onValueChange={(v) => setBulkStatus(v as CorteStatus)}>
                    <SelectTrigger className="w-56 h-8 text-xs"><SelectValue placeholder="Novo progresso..." /></SelectTrigger>
                    <SelectContent>
                      {CORTE_STATUSES.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <Button
                    size="sm"
                    ...
                    onClick={() => {
                      if (bulkStatus === 'Corte') setBulkJustifyOpen(true);
                      else aplicarBulk();
                    }}
                  >
                    Mudar progresso
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card rounded-xl p-6 md:p-8 western-shadow">
                ...
                {/* Botões de ação de corte */}
                <div className="pt-4 mt-4 border-t border-border space-y-2">
                  {order.status === 'Corte' && (
                    <button
                      onClick={() => baixar('Baixa Corte')}
                      disabled={acting}
                    ...
                    >
                      <ArrowRight size={18} /> Marcar BAIXA Corte
                    </button>
                  )}
                  {order.status === 'Baixa Corte' && (
                    <button
                    ...
                    >
                      <RotateCcw size={18} /> Voltar para CORTE
                    </button>
                  )}
                </div>
              </div>
      ...
      <JustificativaDialog
        ...
        description="Voltar de Baixa para Corte é um retrocesso. Descreva o motivo — ficará registrado no histórico."
        onConfirm={async (motivo) => { setJustifyOpen(false); await baixar('Corte', motivo); }}
        onCancel={() => setJustifyOpen(false)}
      />
      <JustificativaDialog
        open={bulkJustifyOpen}
        title="Justificativa do retrocesso em massa"
        description="Pedidos em Baixa serão movidos de volta para Corte. Descreva o motivo."

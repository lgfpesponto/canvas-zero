...
import { Users, RefreshCw, Search, AlertTriangle, Circle, ChevronDown, ChevronRight } from 'lucide-react';
...
function GestaoPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      >
        {/* Banner único de aviso/deploy */}
        <DeployAnnouncementCard />

        <Tabs defaultValue="online" className="mt-2">
          <TabsList>
            <TabsTrigger value="online">Ao vivo</TabsTrigger>
            <TabsTrigger value="auditoria">Auditoria</TabsTrigger>
            <TabsTrigger value="pdfs">Histórico de PDFs</TabsTrigger>
          </TabsList>
            ...
            <Collapsible open={auditoriaOpen} onOpenChange={setAuditoriaOpen}>
              <Card>
                <CollapsibleTrigger asChild>
                  <button type="button" className="flex w-full items-center justify-between p-4 text-left hover:bg-muted/40">
                    <div className="flex items-center gap-2">
                      {auditoriaOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                      <span className="font-medium">Auditoria de alterações</span>
                    </div>
                    <span className="text-xs text-muted-foreground">
                      {auditoriaOpen ? 'ocultar' : 'mostrar'}
                    </span>
                  </button>
                </CollapsibleTrigger>
                <CollapsibleContent>

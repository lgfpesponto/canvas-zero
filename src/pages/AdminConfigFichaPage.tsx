...
import {
  ArrowLeft, Layers, CheckCircle, Plus, Save, ChevronDown, ChevronRight,
  Trash2, ArrowUp, ArrowDown, GripVertical, Link2,
} from 'lucide-react';
              ...
              <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => {
                if (confirm(`Remover categoria "${cat.nome}" e todas suas variações?`)) {
                  deleteCat.mutate(cat.id, {
                    onSuccess: () => { toast.success('Categoria removida'); onDelete(); },
        ...
        <CollapsibleContent>
          <CardContent className="px-4 pb-4 pt-0">
            <div className="space-y-1">
              ...
              {variacoes?.length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-4">Nenhuma variação</p>
              )}
            </div>
            ...
            <Dialog open={addOpen} onOpenChange={setAddOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle className="font-montserrat lowercase">nova variação</DialogTitle>
                </DialogHeader>
...
function CampoSection({
  ...
  const handleReorder = (dir: 'up' | 'down') => {
    ...
    updateCampo.mutate({ id: other.id, ordem: campo.ordem ?? index }, {
      onSuccess: () => onRefetch(),
    });
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="flex gap-0.5">
            <Button size="icon" variant="ghost" className="h-6 w-6" disabled={index === 0} onClick={() => handleReorder('up')}>
              <ArrowUp className="h-3 w-3" />
            </Button>
            <Button size="icon" variant="ghost" className="h-6 w-6" disabled={index === total - 1} onClick={() => handleReorder('down')}>
              <ArrowDown className="h-3 w-3" />
            </Button>
          </div>
        ...
        <div className="flex items-center gap-1">
          <Switch
            checked={campo.ativo ?? true}
            onCheckedChange={checked => updateCampo.mutate({ id: campo.id, ativo: checked }, { onSuccess: onRefetch })}
            className="scale-75"
          />
        ...
        <div className="grid gap-3 sm:grid-cols-2 pt-2 border-t border-border/40">
          <div className="space-y-1">
            ...
            <Input
              ...
              onBlur={e => {
                if (e.target.value !== campo.nome) {
                  updateCampo.mutate({ id: campo.id, nome: e.target.value, slug: slugify(e.target.value) }, { onSuccess: onRefetch });
                }
              }}
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <Select defaultValue={campo.vinculo || ''} onValueChange={v => updateCampo.mutate({ id: campo.id, vinculo: v || null }, { onSuccess: onRefetch })}>
              ...
              <SelectContent>
                {VINCULOS.map(v => <SelectItem key={v.value || 'none'} value={v.value || 'none'}>{v.label}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
            ...
            <Select
              defaultValue={(campo as any).relacionamento?.depende_de || ''}
              onValueChange={v => {
                const newRel = v ? { depende_de: v } : null;
                updateCampo.mutate({ id: campo.id, relacionamento: newRel }, { onSuccess: onRefetch });
              }}
            >
              <SelectTrigger className="h-8"><SelectValue placeholder="Nenhum" /></SelectTrigger>
              <SelectContent>
          ...
          <div className="flex items-center gap-3 col-span-full">
            <div className="flex items-center gap-2">
              <Switch
                ...
                onCheckedChange={v => updateCampo.mutate({ id: campo.id, obrigatorio: v }, { onSuccess: onRefetch })}
              />
            ...
            {campo.tipo === 'checkbox' && (
              <div className="flex items-center gap-2">
                <Switch
                  checked={campo.desc_condicional}
                  onCheckedChange={v => updateCampo.mutate({ id: campo.id, desc_condicional: v }, { onSuccess: onRefetch })}
                />
          ...
          {['selecao', 'multipla'].includes(campo.tipo) && (
            <div className="space-y-1 col-span-full">
              ...
              <Textarea
                rows={4}
                defaultValue={Array.isArray(campo.opcoes) ? (campo.opcoes as any[]).map((o: any) => `${o.label}${o.preco_adicional ? ` | ${o.preco_adicional}` : ''}`).join('\n') : ''}
                className="text-xs"
                onBlur={e => {
...
export default function AdminConfigFichaPage() {
  ...
  const handleReorderCat = (catId: string, direction: 'up' | 'down') => {
    ...
    updateCat.mutate({ id: categorias[swapIdx].id, ordem: categorias[idx].ordem ?? idx }, {
            ...
            <div className="mb-4 flex items-center justify-between">
              ...
              <Dialog open={novoCampoOpen} onOpenChange={setNovoCampoOpen}>
                ...
                <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle className="font-montserrat lowercase">novo campo</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-3 pt-2">
                    ...
                    {['selecao', 'multipla'].includes(novoCampo.tipo) && (
                      <div className="space-y-1">
                        ...
                        <Textarea
                          rows={4}
                          value={novoCampo.opcoesRaw}
                          onChange={e => setNovoCampo(p => ({ ...p, opcoesRaw: e.target.value }))}
                          placeholder={'Opção A | 10.00\nOpção B | 0\nOpção C'}
                        />
                      </div>
                    )}
                    <Button onClick={handleAddCampo} disabled={insertCampo.isPending} className="w-full">
                      Adicionar campo
                    </Button>
                  </div>
                </DialogContent>
            ...
            <div className="space-y-2">
              {campos.map((campo, i) => (
                <CampoSection
                  ...
                  total={campos.length}
                  fichaTipoId={tipo.id}
                  onRefetch={refetchCampos}
                />
              ))}
              {campos.length === 0 && (
                <p className="text-center text-sm text-muted-foreground py-4">Nenhum campo personalizado.</p>
              )}
            </div>
          </section>
        )}
              ...
              {categorias?.map((cat, i) => (
                ...
                >
                  <CategoriaSection
                    ...
                    onDelete={refetchCats}
                    onMoveUp={() => handleReorderCat(cat.id, 'up')}
                    onMoveDown={() => handleReorderCat(cat.id, 'down')}
                    isFirst={i === 0}
                    isLast={i === (categorias?.length ?? 0) - 1}
                  />
                </motion.div>
              ))}
            </AnimatePresence>

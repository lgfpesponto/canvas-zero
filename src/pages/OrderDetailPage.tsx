...
import { ArrowLeft, CheckCircle2, ChevronLeft, ChevronRight, Clock, History, Pencil, ScanBarcode, CheckSquare, Loader2, Printer, Image as ImageIcon, MessageCircle, Send, XCircle } from 'lucide-react';
...
const OrderDetailPage = () => {
  ...
  const detailsGrouped: { categoria: string; itens: [string, string][] }[] = [
    ...
    },
    {
      ...
      itens: filterPairs([
        ...
        ['Recorte Cano', (order as any).recorteCano],
        ['Cor Recorte Cano', (order as any).corRecorteCano],
        ['Laser Gáspea', order.laserGaspea],
        ['Cor Glitter/Tecido Gáspea', order.corGlitterGaspea],
        ['Recorte Gáspea', (order as any).recorteGaspea],
        ['Cor Recorte Gáspea', (order as any).corRecorteGaspea],
        ['Laser Taloneira', order.laserTaloneira],
        ['Cor Glitter/Tecido Taloneira', order.corGlitterTaloneira],
        ['Recorte Taloneira', (order as any).recorteTaloneira],
        ['Cor Recorte Taloneira', (order as any).corRecorteTaloneira],
  ...
  // Auto-correção movida para antes dos early returns (ver acima de `if (orderLoading)`).
        ...
        <div className="bg-card rounded-xl p-6 md:p-8 western-shadow">
          ...
          {order.tipoExtra && order.extraDetalhes ? (
            <div className="mb-6 space-y-4">
              ...
              ) : order.tipoExtra === 'cinto' && extraDetailsGroupedCinto.length > 0 ? (
                <div className="space-y-5">
                  {extraDetailsGroupedCinto.map(grupo => (

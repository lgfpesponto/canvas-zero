        ...
        if ((role === 'bordado' || role === 'corte') && !filters) {
          ...
          while (true) {
            const { data, error } = await supabase
              .from('orders')
              .select('id, numero, status, modelo, tipo_extra, couro_cano, cor_couro_cano, couro_gaspea, cor_couro_gaspea, couro_taloneira, cor_couro_taloneira, extra_detalhes')
          ...
          // Mesma sequência das colunas do portal: 1ª etapa (entrada) e depois a baixa,
          // cada uma agrupada por couro → cor → modelo, com cintos no final.
          const ordered = portalStatuses.flatMap(st =>
            sortOrdersForPrint(all.filter(o => o.status === st))
          );
          if (!cancelled) setIds(ordered.map(o => o.id));
          return;
        }

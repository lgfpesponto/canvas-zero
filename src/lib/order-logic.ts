...
export function dbRowToOrder(row: any): Order {
  return {
    ...
    observacaoEntrega: row.observacao_entrega || undefined,
    observacaoEntregaPor: row.observacao_entrega_por || undefined,
    observacaoEntregaEm: row.observacao_entrega_em || undefined,
...
export function orderToDbRow(order: any, userId: string) {
  return {
    ...
    preco: order.preco ?? 0,
    preco_migrado_v2: order.precoMigradoV2 ?? true,

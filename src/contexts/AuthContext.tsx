...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addOrder = useCallback(async (orderData: Omit<Order, 'id' | 'numero' | 'dataCriacao' | 'horaCriacao' | 'diasRestantes' | 'historico' | 'status' | 'alteracoes'> & { numeroPedido?: string }): Promise<boolean> => {
    try {
      ...
      // Stamp da versão ativa da ficha (bota / cinto / extras). Se a versão
      // não existir para o slug, permanece nulo (compat com pedidos antigos).
      let fichaVersaoId: string | null = (rest as any).fichaVersaoId || null;
      if (!fichaVersaoId) {
        const slugFicha = rest.tipoExtra || 'bota';
        try {
        ...
        } catch (e) { console.warn('addOrder ficha_versao_id lookup', e); }
      }

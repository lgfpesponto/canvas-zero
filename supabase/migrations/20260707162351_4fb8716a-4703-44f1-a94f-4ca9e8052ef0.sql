...
BEGIN
  ...
  UPDATE public.orders
     SET desconto = novo_desconto,
         desconto_justificativa = prefixo || s.motivo,
         preco = novo_preco,
         historico = COALESCE(historico, '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
           ...
           'descricao', 'Ajuste aprovado por ' || COALESCE(admin_nome,'admin') || ': desconto de R$ ' || to_char(valor, 'FM999999990.00') || ' aplicado. ' || prefixo || s.motivo,
  ...
  IF ped.vendedor IS NOT NULL AND ped.vendedor <> '' AND ped.vendedor <> 'Estoque' THEN
    ...
    VALUES (
      ...
      'Ajuste de preço APROVADO: desconto de R$ ' || to_char(valor, 'FM999999990.00') || ' aplicado. ' || prefixo || s.motivo,
      auth.uid()
    );
  END IF;
END;

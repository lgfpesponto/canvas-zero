-- Atualiza comprar_estoque para gravar ficha_snapshot e nome_produto DENTRO de cada bota,
-- permitindo que a tela de detalhes reconstrua a composição da ficha por item.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    FOR i IN 1..v_qtd LOOP
      ...
      v_botas := v_botas || jsonb_build_array(jsonb_build_object(
        ...
        'nome_produto', v_row.nome,
        'ficha_snapshot', COALESCE(v_row.ficha_snapshot, '{}'::jsonb)
      ));
  ...
  PERFORM public.liberar_reservas_usuario();

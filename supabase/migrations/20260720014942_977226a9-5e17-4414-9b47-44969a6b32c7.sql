...
-- Remove overload antigo (5 args) que ainda gravava 'Pendente'
DROP FUNCTION IF EXISTS public.comprar_estoque(jsonb, text, text, text, text);

-- Recria comprar_estoque(6 args) trocando 'Pendente' -> 'Em aberto'
CREATE OR REPLACE FUNCTION public.comprar_estoque(
  ...
  _whatsapp text,
  _numero_pedido text,
  _desconto_aplicado jsonb DEFAULT NULL::jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Backfill: pedidos de estoque que ficaram com status 'Pendente'
UPDATE public.orders
   SET status = 'Em aberto',
       historico = CASE
         WHEN jsonb_typeof(historico) = 'array'
              AND jsonb_array_length(historico) > 0
              AND historico->0->>'local' = 'Pendente'
         THEN jsonb_set(historico, '{0,local}', '"Em aberto"'::jsonb)
         ELSE historico
       END
 WHERE status = 'Pendente'
   AND (extra_detalhes->>'origem_estoque')::boolean IS TRUE;

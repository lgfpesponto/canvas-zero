...
-- Tabelas de descontos de estoque
CREATE TABLE public.estoque_descontos (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  nome text NOT NULL,
  tipo text NOT NULL CHECK (tipo IN ('pct','fixo')),
  valor numeric NOT NULL CHECK (valor > 0),
  escopo text NOT NULL CHECK (escopo IN ('todos','produtos')),
...
GRANT SELECT ON public.estoque_descontos TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.estoque_descontos TO authenticated;
GRANT ALL ON public.estoque_descontos TO service_role;

ALTER TABLE public.estoque_descontos ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Public can read active descontos"
  ON public.estoque_descontos FOR SELECT
  USING (ativo = true);
CREATE POLICY "Authenticated can read all descontos"
  ON public.estoque_descontos FOR SELECT TO authenticated
  USING (true);
CREATE POLICY "Only admin_master can insert descontos"
  ON public.estoque_descontos FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin_master'::app_role));
CREATE POLICY "Only admin_master can update descontos"
  ON public.estoque_descontos FOR UPDATE TO authenticated
  USING (public.has_role(auth.uid(), 'admin_master'::app_role));
CREATE POLICY "Only admin_master can delete descontos"
  ON public.estoque_descontos FOR DELETE TO authenticated
  USING (public.has_role(auth.uid(), 'admin_master'::app_role));

CREATE TRIGGER trg_estoque_descontos_updated
  BEFORE UPDATE ON public.estoque_descontos
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- M:N desconto <-> produto (grupo por nome+raiz_sku)
CREATE TABLE public.estoque_desconto_produtos (
  desconto_id uuid NOT NULL REFERENCES public.estoque_descontos(id) ON DELETE CASCADE,
  produto_grupo_key text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (desconto_id, produto_grupo_key)
);

GRANT SELECT ON public.estoque_desconto_produtos TO anon;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.estoque_desconto_produtos TO authenticated;
GRANT ALL ON public.estoque_desconto_produtos TO service_role;

ALTER TABLE public.estoque_desconto_produtos ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Public can read desconto produtos"
  ON public.estoque_desconto_produtos FOR SELECT
  USING (true);
CREATE POLICY "Only admin_master can manage desconto produtos"
  ON public.estoque_desconto_produtos FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin_master'::app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin_master'::app_role));

CREATE INDEX idx_desconto_produtos_grupo ON public.estoque_desconto_produtos(produto_grupo_key);

-- Coluna snapshot do desconto aplicado no pedido
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS desconto_aplicado jsonb;

-- Atualiza comprar_estoque para aceitar _desconto_aplicado jsonb e gravar
CREATE OR REPLACE FUNCTION public.comprar_estoque(
  ...
  _whatsapp text,
  _numero_pedido text,
  _desconto_aplicado jsonb DEFAULT NULL
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.comprar_estoque(jsonb, text, text, text, text, jsonb) TO authenticated;

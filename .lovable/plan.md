...
2. **Backfill apenas dos produtos de estoque**: para cada `estoque_produtos` cujo snapshot está incompleto, recompor a partir do pedido de origem (`orders.estoque_produto_id`). Isso serve para que as **próximas compras** já saiam com a composição completa. Produtos sem pedido de origem rastreável ficam como estão (nada é apagado).

3. **Pedidos de estoque já criados não são alterados**: nenhum backfill em `orders` — nem no snapshot, nem no preço. Eles continuam exatamente como estão.

4. **Preço do pedido permanece congelado**: o produto de estoque continua acompanhando a ficha atual, mas o pedido de compra mantém o valor da versão da ficha vigente no momento da compra. Novos pedidos passam a gravar no snapshot também os preços por componente do instante da compra, para que a composição continue fiel mesmo depois de mudanças de preço na ficha.
